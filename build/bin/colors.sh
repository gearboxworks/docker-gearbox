...
GB_TERM_CMD="/etc/gearbox/bin/JsonToConfig -json /etc/gearbox/etc/colors.json -template-string"
...
p_err()    { local _name="$1"; shift; setExit 1; blinkon; bp_echo "${_name}" red white $@; }
...
c_err()    { setExit 1; blinkon; bp_echo "" red white "$@"; }
...
checkExit()
{
	...
	then
		blinkon
		bp_echo "" red white "Exited with $EXITCODE - \"$@\""
		setExit "${EXITCODE}"
		exit $EXITCODE
	fi
}
